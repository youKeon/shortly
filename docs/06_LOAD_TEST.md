...
## 4. 2차 개선: 클릭 이벤트 발행을 Virtual Thread로 분리

### 4.1 작업 목표
- P95 및 P90 개선

### 4.2 원인 분석
- Kafka 이벤트 발행 코드 주석 처리 후 동일 테스트 수행, P95 53%, P90 50% 개선 확인
- KafkaProducer의 doSend() 메서드는 직렬화(key, value), 파티션 선택, RecordAccumulator에 레코드 추가 작업이 동기로 진행
```java
...
### 4.3 개선 작업: 이벤트 발행 작업을 메인 스레드에서 분리하여 응답 지연 제거
- 고려한 기술:
  - Redis Buffer
    - 구현 복잡도와 추가 인프라를 고려하면 오버엔지니어링이라고 판단
  - @Async
    - ThreadPoolTaskExecutor 설정 필요
  - Virtual Thread (선택)
    - 별도의 풀 관리가 필요하지 않고, I/O 대기 시 플랫폼 스레드를 점유하지 않아 효율적
