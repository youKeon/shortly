...
export function setup() {
  console.log('');
  console.log('=========================================================');
  console.log('🎯 수평 확장 테스트 (2개 인스턴스) - 10K TPS 목표');
  console.log('=========================================================');
  console.log(`서버 인스턴스: ${SERVERS.length}개`);
  ...
  for (let j = 0; j < totalBatches; j++) {
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      const baseUrl = getServer(); // 서버 분산
      requests.push(['POST', `${baseUrl}/api/v1/urls/shorten`, payload, {
...
export default function (data) {
  ...
  if (isRedirect && data.shortCodes.length > 0) {
    ...
    const res = http.post(`${baseUrl}/api/v1/urls/shorten`, payload, {
      headers: {'Content-Type': 'application/json'},
    });
