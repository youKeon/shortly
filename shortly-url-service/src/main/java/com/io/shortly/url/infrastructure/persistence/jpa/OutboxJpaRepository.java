...
public interface OutboxJpaRepository extends JpaRepository<OutboxJpaEntity, Long> {
     ...
     * 발행 대기 중인 이벤트 조회
     */
    @Query("SELECT e FROM OutboxJpaEntity e WHERE e.status = com.io.shortly.url.infrastructure.persistence.jpa.OutboxEventStatus.PENDING ORDER BY e.createdAt ASC")
