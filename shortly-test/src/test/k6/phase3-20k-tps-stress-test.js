...
/**
 * Phase 3: Stress Test (System Limit Verification)
 *
 * 목표: 20,000 TPS
 * 시간: 10분
...
export const options = {
  ...
  thresholds: {
    // 성능 임계값 (Phase 3: Stress Test - 관대한 기준)
    'http_req_duration': ['p(95)<1000', 'p(99)<2000'],  // P95 < 1s, P99 < 2s
    'http_req_failed': ['rate<0.10'],                   // 에러율 < 10%

    // 기능별 성공률 (낮춤)
    'shorten_success_rate': ['rate>0.90'],              // 단축 성공률 > 90%
    'redirect_success_rate': ['rate>0.90'],             // 리디렉션 성공률 > 90%
    'stats_success_rate': ['rate>0.85'],                // 통계 조회 성공률 > 85%

    // 기능별 응답 시간 (관대함)
    'shorten_duration': ['p(95)<500'],                  // 단축 P95 < 500ms
    'redirect_duration': ['p(95)<200'],                 // 리디렉션 P95 < 200ms
    'stats_duration': ['p(95)<400'],                    // 통계 P95 < 400ms
  },
...
export function setup() {
  ...
  const codes = [];
  const seedCount = 500;  // Phase 3는 가장 많은 Seed Data
  ...
  if (codes.length < 250) {
    throw new Error(`Setup 실패: ${codes.length}개만 생성됨 (최소 250개 필요)`);
  }

  console.log(`   ✓ Seed Data 준비 완료: ${codes.length}개`);
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   테스트 시작 (10분)                         ║');
  console.log('║             Target TPS: 20,000 (1,600/18,000/400)            ║');
  console.log('╚══════════════════════════════════════════════════════════════╝');
  console.log('');
      ...
      'shorten: has shortCode': (r) => {
        try {
          ...
          if (body.shortCode && body.shortCode.length > 0) {
            ...
            // 메모리 관리 (최대 10000개 유지)
            if (globalShortCodes.length > 10000) {
              globalShortCodes = globalShortCodes.slice(-10000);
            }
            return true;
          }
          return false;
...
export function handleSummary(data) {
  ...
  if (tps >= targetTPS) {
    ...
    console.log('⚠️  PARTIAL: 시스템이 목표 TPS에 도달하지 못했습니다.');
    console.log(`   → 현재 시스템 한계: ${tps.toFixed(0)} TPS`);
    console.log('   → 추가 최적화 또는 스케일 아웃 필요');
  }
  console.log('');
  ...
  return {
    ...
    'phase3-result.json': JSON.stringify({
      phase: 3,
      target: {
        tps: targetTPS,
        testType: 'stress',
