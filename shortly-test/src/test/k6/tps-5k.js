...
/**
 * 목표: 5,000 TPS (Redirection 4,500 / Shorten 400 / Stats 100) <br>
 * 시간: 6분
 */
const TARGET_TPS = 5000;
const TEST_DURATION_SEC = 360; // 6분
...
export const options = {
  ...
  thresholds: {
    ...
    shorten_success_rate: ['rate>0.96'],
    redirect_success_rate: ['rate>0.97'],
    stats_success_rate: ['rate>0.95'],
    shorten_duration: ['p(95)<280'],
    redirect_duration: ['p(95)<90'],
    stats_duration: ['p(95)<180'],
  },
...
export function setup() {
  console.log('\n===== 5K TPS Validation: Setup Started =====');
  ...
  if (codes.length < 120) {
    throw new Error('Seed generation failed (less than 120 codes)');
  }

  console.log(`Seed prepared: ${codes.length}`);
  console.log('===== Setup Complete. Test Running for 6 minutes =====\n');
...
export function teardown(data) {
  ...
  console.log(`\n===== 5K TPS Validation Completed (${Math.floor(elapsed / 60)}m ${elapsed % 60}s) =====\n`);
}
...
export function handleSummary(data) {
  ...
  return {
    ...
    'summary-5k.html': htmlReport(data),
    'tps-5k-result.json': JSON.stringify({
